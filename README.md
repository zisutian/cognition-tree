...
日常可用目录 `repo` 中运行 `./start.sh`，初始地址为 `http://127.0.0.1:3001`。
可用版需要 Bash 和 Node.js 22.18.0 或更高版本，已经包含运行依赖。

完整源码仓库位于 `repo-dev`。在该目录运行 `./start.sh` 启动独立开发环境，初始地址为
`http://127.0.0.1:3002`；开发需要 pnpm 11.1.3 与 Git。两个入口均不接受模式参数。
开发数据与正式数据独立。Ollama 或其他模型服务是可选项。
