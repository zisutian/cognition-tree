...
import {
  ...
  type SystemRepositoryIssueView,
  type SystemRepositoryOption,
...
function SystemRepositoryIcon({
  purpose,
}: {
  purpose: SystemRepositoryPurpose;
}) {
  const Icon = purpose === "system-journal" ? CalendarDays : ListChecks;
...
function selectedRepositoryTarget(
  ...
  switch (selection.kind) {
    ...
    case "system-repository":
      return {
        issue: view.systemIssues.find(({ id }) => id === selection.id) ?? null,
        kind: selection.kind,
        purpose: selection.id,
        repository: view.systemRepositories.find(({ id }) =>
          id === selection.id
        ) ?? null,
      } as const;
  }
  ...
  useEffect(() => {
    ...
    const target = Array.from(
      contextRef.current?.querySelectorAll<HTMLElement>(
        "[data-repository-catalog], [data-repository-issue-id], [data-repository-id], [data-system-repository-id]",
      ) ?? [],
    ).find((element) => {
      switch (focusRequest.kind) {
        ...
        case "system-repository":
          return element.dataset.systemRepositoryId === focusRequest.id;
      }
    });
  ...
  return (
    ...
    >
      <CompactContextGroup
        ...
        count={systemPurposes.length}
        headingId="repository-group-system"
        label="内置"
        listClassName="repository-list"
      >
        {systemPurposes.map((purpose) => {
          const repository = view.systemRepositories.find(({ id }) =>
            id === purpose
          );
          const issue = view.systemIssues.find(({ id }) => id === purpose);
          const selected = currentSelection.kind === "system-repository" &&
            currentSelection.id === purpose;
          const hasProblem = Boolean(
            issue ||
              repository?.hasProblem ||
              view.systemCatalogStatus === "failed",
          );
          const status = issue
            ? "故障"
            : repository?.hasProblem
          ...
          return (
            <CompactContextRow
              buttonProps={{ "data-system-repository-id": purpose }}
              icon={<SystemRepositoryIcon purpose={purpose} />}
              key={purpose}
              ...
              selected={selected}
              title={`${systemRepositoryLabel(purpose)} · 内置受保护仓库`}
              trailing={(
                <>
                  <span className={cx(
                  ...
                  {hasProblem ? (
                    <AlertTriangle
                      aria-label={`${systemRepositoryLabel(purpose)}仓库存在问题`}
...
function SystemRepositoryDetail({
  busy,
  issue,
  purpose,
  repository,
  ...
  busy: boolean;
  issue: SystemRepositoryIssueView | null;
  purpose: SystemRepositoryPurpose;
  repository: SystemRepositoryOption | null;
  view: RepositoryViewModel;
  onCopy: (label: string, value: string) => void;
      ...
      <>
        ...
        <Section className="repository-section" title="操作">
          <div className="repository-operation-strip">
            <Button
              disabled={busy || view.retryingSystemPurpose !== null}
              onClick={() => onRunAction(() =>
  ...
  if (!repository) {
    return (
      <EmptyState
        action={view.systemCatalogStatus === "failed"
          ? (
            <Button
              disabled={busy}
              onClick={() => onRunAction(view.reloadSystemCatalog)}
              type="button"
              variant="secondary"
            >
              重试内置仓库
          ...
          : undefined}
        description={view.systemCatalogErrorMessage || "内置仓库正在载入。"}
        title={systemRepositoryLabel(purpose)}
      />
    );
  }
  ...
  const title = target.kind === "create"
    ...
    : target.kind === "ordinary-repository"
      ...
      : target.kind === "ordinary-issue"
        ...
        : systemRepositoryLabel(target.purpose);
    ...
    <Panel className="repository-panel" aria-label="仓库">
      ...
      <PanelBody scroll>
        <div className="repository-content-column">
          ...
          {target.kind === "system-repository" ? (
            <SystemRepositoryDetail
              busy={busy}
              issue={target.issue}
