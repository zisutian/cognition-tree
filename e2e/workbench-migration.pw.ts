...
test("migrates real files through settings and reloads the durable migration status", async ({
  api,
  page,
  e2eServer,
  repositoryRoot,
  ...
  const before = await (
    await api.get(
      buildApiOperationPath("getWorkspaceSyncSnapshot", { repositoryId }),
    )
  ).json();
  ...
  await panel
    ...
    .fill(e2eServer.migrationDestination);
  ...
  await expect
    .poll(async () => {
      const response = await api.get(
        buildApiOperationPath("getCurrentDataRootMigration"),
      );
  ...
  const relativeFiles = await readdir(path.join(repositoryRoot, repositoryId), {
    recursive: true,
    withFileTypes: true,
  });
  for (const entry of relativeFiles.filter(
    ...
    expect(
      await readFile(path.join(e2eServer.migrationDestination, relative)),
      relative,
    ).toEqual(await readFile(source));
  }
});

test("rejects an occupied migration destination without removing its contents", async ({
  api,
  page,
  e2eServer,
}) => {
