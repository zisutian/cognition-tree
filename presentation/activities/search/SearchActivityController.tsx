...
export function SearchActivityController({
  ...
  const repositories =
    application.repository.catalogState.status === "ready"
      ? application.repository.catalogState.repositories.map(
          ({ id, label }) => ({
            id,
            label,
          }),
  ...
  const openResult = (result: SearchResult) => {
    const activity = result.domain === "workspace" ? "notes" : result.domain;
...
export type SearchActivityApplication = {
  repository: Pick<RepositoryApplication, "catalogState">;
  search: {
...
export type SearchActivityControllerProps =
  ActivityControllerProps<SearchActivityApplication>;
