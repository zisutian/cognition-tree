...
Workspace：
    ...
    零个或多个普通笔记库。每个仓库独立保存笔记、目录、多份语法和工作台偏好。Local 与 WebDAV adapter 之间不自动复制或合并内容。
