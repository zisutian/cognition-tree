...
失败必须显示为可重试错误，不能解释成整仓冲突。冲突期间普通编辑保持开放，冲突单元随已接受的编辑重新计算；全部冲突消除后自动
恢复同步，无需额外点击。另存本地必须证明覆盖全部将被远端选择丢弃的冲突单元，任何不支持或混合单元都在主内容
...
逐文件校验后最后切换 bootstrap 指针。未提交失败继续使用源；提交结果不确定时保持
维护状态并自动对账，无法证明权威目录时提供诊断与重新对账入口。刷新与进程重启后
恢复同一迁移状态。源目录与本次已分配的目标目录均保留，不提供绕过校验的强制恢复。
