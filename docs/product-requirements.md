...
不能创建本地空仓库。显式冲突必须包含可恢复的 base、本地、远端和冲突单元快照，
只获知远端 revision 不得伪装成冲突。保留本地、采用远端和另存本地都必须冻结编辑，
以当前 local/remote revision exact proof 解决并直接采用返回的最终 snapshot；详情读取
失败必须显示为可重试错误，不能解释成整仓冲突。冲突发布后普通编辑保持关闭；另存
本地必须证明覆盖全部将被远端选择丢弃的冲突单元，任何不支持或混合单元都在主内容
写入前整体失败。精确 CAS、三方合并和请求期间继续编辑的算法由架构边界定义。
