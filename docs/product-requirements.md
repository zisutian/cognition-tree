...
Todo 不包含截止日期、优先级、备注、筛选或跨集合任务移动。Journal 跨仓引用不进入普通仓库引用图谱。远程客户端统一使用 /api/v1 资源查询和领域命令；本项目不再提供专用手机契约，完整同步只授权官方客户端。当前项目不提供 Dockerfile、镜像或 Compose；只保留未来容器路径约定。
