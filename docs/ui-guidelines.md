...
仓库：
    ...
    选中普通仓库后才显示“开”和内联重命名；主区标题承担名称并只承载恢复、冲突处理、确认和危险操作。右侧状态区以选中名称分组，并用共享属性表展示状态、ID、位置、路径、故障和冲突事实，复制按钮进入属性操作列；健康活动仓库的文件重扫只出现在笔记工具栏。
...
其他：
    ...
    服务状态栏区分当前配置、待重启状态、下一访问地址和数据根；owner secret 只展示一次。chat Profile 字段固定命名为“会话历史预算（字符）”；Ollama 推理强度显示模型默认、关闭、低、中、高，其他 OpenAI-compatible Profile 不显示该字段。Provider 探测与符合性检查使用结构化状态摘要。Codex 认证只显示当前选择的 API Key 或 ChatGPT 设备码流程。API 访问分别管理 automation 与 trusted-client token；审计刷新位于标题栏，记录使用共享管理列表，技术 hash 位于右侧状态栏。
