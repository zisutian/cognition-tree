...
export class WriteAdmissionClosedError extends Error {
  constructor() { super("Data-root maintenance is active"); this.name = "WriteAdmissionClosedError"; }
}
...
export class ApplicationWriteBarrier {
  ...
  async begin(): Promise<WriteLease> {
    if (this.#exclusive) throw new WriteAdmissionClosedError();
    this.#exclusive = true;
    if (this.#active > 0) await new Promise<void>((resolve) => this.#waiters.add(resolve));
    let finished = false;
    return { finish: () => {
      if (finished) return;
      finished = true;
