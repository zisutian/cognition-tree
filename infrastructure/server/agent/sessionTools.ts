...
import {
  AgentJournalCommandIntentSchema,
  AgentTodoCommandIntentSchema,
  AgentWorkspaceCommandIntentSchema,
...
export const agentRuntimeTools = agentToolDefinitions.map(toRuntimeTool);
...
export class AgentSessionTools {
  ...
  async execute(
    ...
    switch (definition.name) {
      ...
      case "stage_workspace_command":
        return {
          result: await this.#stageWorkspace(
            record,
            parseAgentSchema(AgentWorkspaceCommandIntentSchema, input),
          ),
        };
      case "stage_journal_command":
        return {
          result: await this.#stageJournal(
            record,
            parseAgentSchema(AgentJournalCommandIntentSchema, input),
          ),
        };
      case "stage_todo_command":
        return {
          result: await this.#stageTodo(
            record,
            parseAgentSchema(AgentTodoCommandIntentSchema, input),
          ),
        };
